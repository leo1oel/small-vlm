...
from .config.config_schema import AppConfig, ModeConfig, ModelConfig, DatasetConfig, TrainerConfig
...
def load_model(cfg: ModelConfig) -> VLM:  # type: ignore
    print_model(cfg)
    model = VLM(cfg)  # type: ignore
    return model
...
def main(cfg: AppConfig) -> None:
    cfg = OmegaConf.structured( OmegaConf.to_object(cfg), AppConfig)
    vlm(cfg)
