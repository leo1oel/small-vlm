...
- **The `input_ids.shape[1] == 1` splice guard must stay cache-gated.** `prepare_inputs_labels_for_multimodal` (`modeling_vlm.py`) short-circuits the connector+splice for a cached single-token DECODE step. The guard is `input_ids.shape[1] == 1 AND past_key_values is not None` — gating on the cache is load-bearing: a bare-`<image>` caption prompt is `[[-200]]` (shape `(1,1)`) at PREFILL with `past_key_values is None`, so the bare `shape[1] == 1` form fired at prefill too, dropped the image, and `generate()` ran blind from the LM prior (the `plain`-template caption probe collapsed to an image-invariant string like `:::::`). Prefill (cache None) must splice; only a real decode step (cache present) skips the re-splice. Do NOT simplify back to `or input_ids.shape[1] == 1`. Training never tripped it (the answer text follows the image, so `shape[1] ≫ 1`); only single-token generation prompts hit it, so chat/MCQ benchmark prompts were unaffected — but any `plain`-template single-image caption generation is. Pinned by `test_single_token_image_prefill_splices_then_decode_short_circuits`; mirrored in `templates/modeling_vlm.py.j2`.
- **Qwen3-backbone native checkpoints serialize `bos_token_id=None`.** The base Qwen3 LM carries `bos_token_id=151643` (and `pad_token_id=None`), but the trained VLM config drops bos to `None`. HF `generate()` needs a bos to seed its output-id bookkeeping whenever it cannot start the sequence from `inputs_embeds`, and raises `ValueError: bos_token_id has to be defined`. On the current path this stays latent (the splice always yields a real `inputs_embeds`, and transformers 5.10.1 returns an empty tracking tensor for models with `inputs_embeds` forwarding before reaching the bos check), but it is one HF-version/code change — or a regression of the splice guard above — away from firing. `load_model` (`eval.py`) pins a valid bos (`pad_token_id`, else `eos_token_id`) when it is `None`; bookkeeping only (never prepended on the `inputs_embeds` path, stripped on decode), and defined-bos backbones are left untouched. Pinned by `test_load_model_pins_bos_for_none_bos_qwen3`.
