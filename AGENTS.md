...
- **The CI `build` job fails on basedpyright WARNINGS, not just errors.** `devtools/lint.py` runs `basedpyright --level error --stats src tests devtools` and treats any nonzero exit as failure; basedpyright still exits nonzero while *surviving* warnings remain, so a clean `0 errors, N warnings` summary turns the build red. ruff and the `pre-commit` workflow can be fully green while this step fails.
- **New post-merge warnings are silenced by extending `[tool.basedpyright]` in `pyproject.toml`** (the PR #16 baseline pattern), not by relaxing `--level error`. Add the specific rule next to its siblings; keep error-level checking intact.
