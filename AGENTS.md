...
- **Qwen ChatML delimiter unmasking collapses the prefix (THE cross-cutting trap).**
  `preprocess_qwen` globally unmasks the ChatML delimiters (`<|im_start|>` / `<|im_end|>` / newline) into the labels, *including* the leading system `<|im_start|>` at position 0.
  ...
  `vlm.py` computes that skip set at load time — ONLY for `qwen*` trainer versions, since other templates supervise answer tokens only — and stows it on `config.cross_modal_prefix_skip_ids`; `install_xmodal_masks` reads it.
...
- **Auto-resume skips optimizer-less checkpoints.** `save_only_model` (legacy pretrain) checkpoints carry no optimizer/scheduler state; `_checkpoint_is_resumable` (`train.py`) refuses to auto-select them so a resume can't silently restart the optimizer while advancing the step counter (explicit `resume_from_checkpoint` still opts in).
