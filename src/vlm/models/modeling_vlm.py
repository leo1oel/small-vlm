...
def create_dynamic_causal_vlm_class(
    ...
    def __init__(self: Any, config):  # pyright: ignore
        ...
        # for the input-word-dropout ramp. Registered ONLY when enabled, so the
        # baseline state_dict is unchanged (audio / visual_aux pattern).
        # Non-persistent: it is a transient mirror of state.global_step (re-synced
        # every step by VLMTrainer._sync_caption_dropout_step), not trained state
        # — so it stays out of checkpoints and never collides across stages (S2
        # disables the dropout and so never registers it).
        if bool(getattr(config, "caption_token_dropout_enabled", False)):
