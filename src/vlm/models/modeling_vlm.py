...
def caption_token_dropout_rate(step: int, max_steps: int, p_start: float, p_end: float) -> float:
    """Linear ramp p_start -> p_end over the optimizer-step budget (exp10 S1
    caption-token dropout). ``step`` is the rank-identical HF Trainer
    ``state.global_step`` (mirrored into the model buffer by
    ``VLMTrainer._sync_caption_dropout_step``), NOT a per-microbatch forward
    counter — a microbatch counter advances at grad_accum x the rate and
    diverges across DDP ranks whose per-step batch counts differ. The fraction
    is clamped to [0, 1]; max_steps <= 0 -> full ramp (frac = 1)."""
    frac = min(1.0, max(0.0, step / max_steps)) if max_steps > 0 else 1.0
    return p_start + (p_end - p_start) * frac


def caption_token_dropout_prob(config: Any, step: int) -> float:
    """Resolve the current drop probability from the (flattened) model config +
    the mirrored optimizer step. Returns 0.0 when disabled, so
    ``apply_caption_token_dropout`` no-ops and the forward is bit-identical to
    the baseline."""
    if not bool(getattr(config, "caption_token_dropout_enabled", False)):
        return 0.0
    return caption_token_dropout_rate(
        int(step),
        int(getattr(config, "caption_token_dropout_max_steps", 0) or 0),
        float(getattr(config, "caption_token_dropout_p_start", 0.0)),
        float(getattr(config, "caption_token_dropout_p_end", 0.0)),
    )


def apply_caption_token_dropout(
    ...
    *,
    ...
    """Blank (zero) a Bernoulli(p) fraction of the INPUT caption-content
    embeddings — the supervised positions where ``labels != ignore_index`` —
    leaving ``labels`` untouched, so every position stays supervised while the
    model is blinded to a fraction of its own preceding caption words (exp10).

    INPUT side only and out-of-place: kept positions retain their autograd
    graph, dropped positions become zero constants (zero gradient) — the repo's
    "blank" idiom, identical to the grounding ``corruption="blank"`` ablation,
    so there are no new parameters and no tokenizer dependency. Image / audio /
    query / BOS / prompt tokens carry ``ignore_index`` labels, so they are never
    eligible to drop. p <= 0 -> returns ``inputs_embeds`` unchanged."""
    if p <= 0.0:
        return inputs_embeds
    supervised = labels != ignore_index
    rand = torch.rand(labels.shape, device=inputs_embeds.device, generator=generator)
    drop = supervised & (rand < p)
    return inputs_embeds.masked_fill(drop.unsqueeze(-1), 0.0)
...
def create_dynamic_causal_vlm_class(
    ...
    def __init__(self: Any, config):  # pyright: ignore
        ...
        # S1 caption-token dropout (exp10): rank-identical optimizer-step mirror
        # for the input-word-dropout ramp, re-synced every step by
        # VLMTrainer._sync_caption_dropout_step. Registered UNCONDITIONALLY: it is
        # a NON-PERSISTENT transient mirror of state.global_step (never trained
        # state), so it stays out of the state_dict / checkpoints and the
        # disabled-arm baseline is bit-identical regardless. The forward gates on
        # caption_token_dropout_prob(), which returns 0.0 when disabled, so the
        # buffer is inert unless an arm opts in. Gating registration on the
        # enabled flag would be unsound: vlm.vlm() flattens that flag onto
        # model.config AFTER load_model() has already run __init__, so on a fresh
        # build the gate would never fire and the mirrored step would be stuck at 0
        # (the ramp pinned at p_start) while a resume — which reads the flag from
        # checkpoint config.json at construction — would register it and diverge.
        self.register_buffer(
            "_caption_dropout_step", torch.zeros((), dtype=torch.long), persistent=False
        )
        self.post_init()
    ...
    def _build_visual_distill_head(self: Any, config: Any) -> nn.Module | None:
        ...
        return VisualDistillHead(
            ...
            loss_type=loss_type,
            num_query=int(getattr(config, "learnable_query_num_query", 64)),
    ...
    def _build_learnable_query(self: Any, config: Any) -> "nn.Parameter | None":
        ...
        single-pool ST-3). randn(num_query, hidden); None when disabled. It is the
        distillation SITE: spliced in at "<query>" placeholders, routed to the
        visual FFN expert, label-masked (no CE), and aligned to the single
        adaptive-pooled CLIP grid by the simplified breen distill method."""
        if not bool(getattr(config, "learnable_query", False)):
            return None
        n = int(getattr(config, "learnable_query_num_query", 64))
    ...
    def _apply_caption_token_dropout(
        self: Any, inputs_embeds: Tensor | None, labels: Tensor | None
    ) -> Tensor | None:
        """Stage-1 caption-token (input word) dropout (exp10). Applies only on
        the training-loss path (``self.training`` and ``labels`` present), after
        the multimodal splice, so it sees the FINAL teacher-forcing inputs +
        labels. Blanks a Bernoulli(p) fraction of the supervised caption INPUT
    ...
    def forward(
        ...
        # S1 caption-token (input word) dropout (exp10): blank a ramped fraction
        # of the model's own supervised caption INPUT embeddings (labels intact)
        # so it must read the image. Applied at the single chokepoint AFTER the
        # splice / mask / expert-routing setup and BEFORE both loss-path
        # dispatches (chunked-CE and the parent forward), and a no-op outside
        # training / when disabled (bit-identical baseline).
        inputs_embeds = self._apply_caption_token_dropout(inputs_embeds, labels)
    ...
    def compute_breen_distill_loss(
        ...
        """BREEN query distillation (spec 2026-06-24; SINGLE-POOL, ST-3 audit §5).

        Gather the LLM final post-norm hidden at each image's learnable-query
        positions (in row order, tagged by query_block_ids), and align ALL
        num_query rows to ONE adaptive avg-pool of the frozen CLIP grid down to a
        √num_query × √num_query grid (no fine/coarse two-pool split; the head's
        norm_layer projects the CLIP target up to LLM-hidden and 1-cos is taken
        there, with anti-collapse A — EMA per-channel debias — applied to the
        target inside compute_breen when enabled). Each query block is paired with
        its image by nearest-preceding image block in
        ...
        # SINGLE-POOL (ST-3 audit §5): one adaptive avg-pool of the CLIP grid down
        # to a √num_query × √num_query grid = num_query target rows. num_query must
        # be a perfect square (the queries align to a square pooled grid). Unlike
        # the old (side//3)/(side//4) two-pool assertion, adaptive_avg_pool2d
        # handles ANY teacher grid side (e.g. CLIP-B/16-224 -> 14x14 -> 8x8), so
        # the arm-9 CLIP-base teacher no longer needs CLIP-L/14-336's 24x24 grid.
        side = int(grids.shape[1])
        side_q = int(round(head.num_query**0.5))
        if side_q * side_q != head.num_query:
            raise ValueError(
                f"breen single-pool distill expects a perfect-square num_query, "
                f"got learnable_query.num_query={head.num_query}; set it to k*k "
                "(e.g. 64 = 8x8) so the queries align to a square pooled CLIP grid."
            )
        if side_q > side:
            # Guard against UPSAMPLING: adaptive_avg_pool2d to a grid LARGER than the
            # teacher's silently replicates cells, so distinct query rows would get
            # identical (degenerate) targets. Single-pool only ever DOWN-pools (8x8
            # from 14x14, 10x10 from 24x24); a larger num_query is a misconfig.
            raise ValueError(
                f"breen single-pool distill would UPSAMPLE: num_query={head.num_query} "
                f"-> {side_q}x{side_q} target grid exceeds the {side}x{side} teacher "
                f"grid ({side * side} cells). Lower num_query to <= {side * side} (a "
                "perfect square) or use a higher-resolution teacher_out_size."
            )
        ...
        for k, flat_pos in blocks:
            ...
            c = g.shape[1]
            pooled = nn.functional.adaptive_avg_pool2d(g, (side_q, side_q))  # (1, C, sq, sq)
            samples.append(
                {
                    "query_hidden": flat_hidden[flat_pos],
                    "target": pooled.reshape(c, -1).t().to(mdtype),  # (num_query, C)
                }
            )
    ...
    def floating_point_ops(
        ...
        # into num_query learnable-query rows. The sentinel itself is already
        # counted in `tokens`; add the remaining rows so total_flos reflects the
        # real spliced length.
        if bool(getattr(self.config, "learnable_query", False)):
            ...
            if n_q:
                rows = int(getattr(self.config, "learnable_query_num_query", 64))
    ...
    DynamicCausalVLMClass = type(
        ...
        (ParentCausalLLMClass,),  # Inherit from the specific LLM class
        {
            ...
            "_apply_caption_token_dropout": _apply_caption_token_dropout,
            "forward": forward,
