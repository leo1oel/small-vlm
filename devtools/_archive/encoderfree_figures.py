...
def fig_fdi():
    ...
    for y, (k, v) in zip(ys, rows):
        ...
        ax[0].text(
            v["func"]["com_rel"], y + 0.18, f"{v['func']['com_rel']:.2f}", ha="center", fontsize=8
        )
    ax[0].axvspan(0.45, 0.55, color="gray", alpha=0.15)
    ax[0].axvline(0.33, color="k", ls=":", lw=0.7)
    ax[0].axvline(0.66, color="k", ls=":", lw=0.7)
    ax[0].set_yticks(ys)
    ax[0].set_yticklabels([k for k, _ in rows], fontsize=9)
    ax[0].set_xlim(0, 1)
    ax[0].set_xlabel("functional fusion CoM (relative depth) = FDI")
    ax[0].set_title(
        "WHERE the image is USED for the answer\n(blue=encoder-free, orange=encoder; gray=mid 0.45-0.55)"
    )
    ax[0].invert_yaxis()
    ...
    for k, v in rows:
        ...
        c = FREE if v["family"] == "free" else ENC
        ax[1].plot(x, rn, "-", lw=1.5, color=c, alpha=0.8, label=f"{k} ({f['com_rel']:.2f})")
    ax[1].axhline(0.5, color="k", lw=0.6, ls="--", alpha=0.4)
    ax[1].axvspan(0.45, 0.55, color="gray", alpha=0.12)
    ax[1].set_xlabel("relative blocked depth d/N")
    ax[1].set_ylabel("retained(d)/R0")
    ax[1].set_title("usable image signal surviving text->image block [0..d)")
    ax[1].legend(fontsize=6.5, ncol=1, loc="upper right")
    fig.tight_layout()
    fig.savefig(OUT / "fig_fdi.png", dpi=130)
    print("wrote fig_fdi.png")
...
def fig_maturation():
    ...
    for k, v in rows:
        ...
        ui = m["u_img"]
        mu = mean(ui) or 1e-9
        ax[0].plot(x, [u / mu for u in ui], "-", color=c, alpha=0.75, lw=1.4, label=f"{k}")
        ut = m["u_txt"]
        mt = mean(ut) or 1e-9
        ax[1].plot(x, [u / mt for u in ut], "-", color=c, alpha=0.75, lw=1.4)
    ax[0].set_title(
        "VISUAL pathway: image-token update u_img(l)/mean\n(higher early = image being 'encoded' in early layers)"
    )
    ax[1].set_title("TEXT pathway: text-token update u_txt(l)/mean")
    for a in ax[:2]:
        a.set_xlabel("relative depth l/N")
        a.axvspan(0, 0.25, color="gray", alpha=0.10)
    ...
    for k, v in rows:
        ...
        ax[2].text(
            v["mat"]["img_com_rel"], v["func"]["com_rel"] + 0.012, k, fontsize=7, ha="center"
        )
    ax[2].plot([0, 1], [0, 1], "k--", lw=0.5, alpha=0.4)
    ...
    fig.tight_layout()
    fig.savefig(OUT / "fig_maturation.png", dpi=130)
    print("wrote fig_maturation.png")


def fig_attention():
    files = {
        "LLaVA-1.5-7B": ("results_attn_llava_full.json", "llava-hf/llava-1.5-7b-hf", "enc"),
    ...
    if am.exists() and am.stat().st_size > 2:
        ...
        for st in ("SFT", "MT"):
            if st in d:
                cl = d[st]["classes"]
                li_, vi = cl.index("last"), cl.index("vision")
    ...
    ax[0].axvspan(0.10, 0.30, color="green", alpha=0.08)
    ax[0].axvspan(0.60, 0.80, color="red", alpha=0.06)
    ax[0].set_xlabel("mean last-token -> image attention fraction")
    ax[0].set_title(
        "FastV attention (blue=encoder-free, orange=encoder)\n"
        "green=SAIL-paper modular 10-30%, red=their native 60-80%\n*NEO on POPE"
    )
    ax[1].set_xlabel("relative depth l/N")
    ax[1].set_ylabel("last->image fraction")
    ax[1].set_title("per-layer last->image")
    ax[1].legend(fontsize=7)
    fig.tight_layout()
    fig.savefig(OUT / "fig_attention.png", dpi=130)
    print("wrote fig_attention.png")
