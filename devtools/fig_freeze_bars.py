...
def main():
    ...
    for name, fam, f in MODELS:
        ...
        for mode in ("img", "txt"):
            r0k = acc(lambda r: r[f"freeze_{mode}"][j2]) - acc(
                lambda r: r[f"freeze_{mode}_null"][j2]
            )
            rec[mode] = max(r0k, 0) / R0
        data.append(rec)
        print(
            f"{name:14} ({fam}) R0={R0:+.2f} | retention@~0.2N: freeze_img={rec['img']:.2f} freeze_txt={rec['txt']:.2f}"
        )
    ...
    for ax, mode, title, sub in (
        (
            axes[0],
            "img",
            "Test 1: FREEZE the IMAGE stream (first 20% of layers)",
            "question: does the LLM itself need to process image tokens?",
        ),
        (
            axes[1],
            "txt",
            "Test 2: FREEZE the TEXT stream (first 20% of layers)",
        ...
        ax.set_xticks(x)
        ax.set_xticklabels(
            [f"{d['name'].replace('-7B', '').replace('-12B', '')}\n{d['fam']}" for d in data],
            fontsize=9,
        )
    ...
    fig.suptitle(
        "Causal check of the labor map: cancel one stream's early in-LLM processing, "
        "measure how much usable image signal survives",
        fontsize=10.5,
        y=1.02,
    )
    fig.tight_layout()
