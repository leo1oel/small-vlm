...
def suf_q(path):
    ...
    if not s:
        return None
    return {"q50": s["q50"], "q25": s["q25"], "q75": s["q75"], "R0": pq.get("R0"),
...
def main():
    ...
    for tag, (label, fam, moe, note) in MODELS.items():
        ...
        if wb.exists():
            d = json.load(open(wb))
            e["worldbench"] = {"q50": d["overall"]["q50"], "R0": d["overall"]["R0"],
                               "by_domain": {k: {"q50": v["q50"], "R0": v["R0"], "n": v["n"]}
                                             for k, v in d["by_domain"].items()}}
        out["models"][tag] = e
        v = e["vmcbench"]
        print(f"{tag:11s} {label:22s} {fam:13s} VMC={v['q50'] if v else '-'} "
              f"MMStar={e['mmstar']['q50'] if e['mmstar'] else '-'} "
              f"WB={e.get('worldbench', {}).get('q50', '-')}", flush=True)
