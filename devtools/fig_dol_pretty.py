...
def setup_font():
    ...
    for f in glob.glob(
        ...
        if "Cond" not in f and "Italic" not in f:
            ...
            added = True
    for f in glob.glob(
        "/gscratch/krishna/leoym/fonts/gillius_ctan/gillius/opentype/GilliusADF-Bold.otf"
    ):
        font_manager.fontManager.addfont(f)
    if added:
...
def fusion_quants(rows):
    ...
    return {
        p: q(v)
        for p, v in (("q10", 0.10), ("q25", 0.25), ("q50", 0.50), ("q75", 0.75), ("q90", 0.90))
    }
...
def main():
    ...
    items = [
        ("grad", cm_img, "image-stream work / layer", 0.13),
