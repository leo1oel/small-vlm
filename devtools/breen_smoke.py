...
def build(device, base_lm, enabled=True, gate=True, from_pretrained=None, weight=1.0):
    ...
    model_cfg = OmegaConf.structured(
        ModelConfig(
            ...
            visual_expert=VisualExpertConfig(enabled=enabled, layers=None, init_from_text=True, gate=gate),
...
def main():
    ...
    check("build: learnable_query Parameter present",
    ...
    print("DIAG learnable_query finite:", bool(torch.isfinite(model.learnable_query).all()),
          "absmax=", float(model.learnable_query.abs().max()), flush=True)
    model.eval()
    bad = []
    hs = [layer.register_forward_hook(
        (lambda li: (lambda m, i, o: bad.append(li)
                     if not torch.isfinite(o[0] if isinstance(o, tuple) else o).all() else None))(li))
        for li, layer in enumerate(model.model.layers)]
    with torch.no_grad():
        oev = model(input_ids=di, attention_mask=da_, images=dim, image_position_ids=dps)
    for h in hs:
        h.remove()
    print("DIAG first non-finite decoder layers:", sorted(set(bad))[:5],
          "| eval logits finite:", bool(torch.isfinite(oev.logits).all()), flush=True)
    model.train()
    ...
    print("DIAG CE-only(train,no-distill) finite:", bool(torch.isfinite(oce.loss)),
          "val=", float(oce.loss), flush=True)
    ...
    check("(b) query rows == num_query per image (100 x 2)",
          n_q_rows == nq * len(images), f"{n_q_rows} vs {nq * len(images)}")
    check("(b) query positions are label-masked (CE ignores them)",
          bool((new_labels[q_pos] == ign).all()))
    check("(b) image positions are label-masked",
          bool((new_labels[img_pos] == ign).all()))
    # The answer tokens (the only non-ignored labels) must survive.
    check("(b) answer tokens still supervised",
          int((new_labels != ign).sum()) == len(ans), f"{int((new_labels != ign).sum())} vs {len(ans)}")
    # The visual-expert routing mask = image OR query positions.
    routed = img_pos | q_pos
    check("(b) routing mask covers image AND query positions",
          int(routed.sum()) == int(img_pos.sum()) + n_q_rows)

    # ---- (a) mechanics + grads ------------------------------------------
    out = model(input_ids=iid, attention_mask=attn, labels=lab, images=images, image_position_ids=posl)
    comps = getattr(model, "_last_ce_components", {})
    check("(a) finite loss", torch.isfinite(out.loss).item(), f"loss={out.loss.item():.4f}")
    check("(a) distill + distill_cos stashed", "distill" in comps and "distill_cos" in comps,
          f"keys={sorted(comps)}")
    out.loss.backward()
    ...
    for step in range(args.steps):
        ...
        if step % 30 == 0:
            print(f"  step {step:3d}: loss={float(o.loss):.4f} distill={d_last:.4f} cos={cos_last:.4f}",
                  flush=True)
    check("(a) distill loss decreased", d_last < d0 - 1e-3, f"{d0:.4f} -> {d_last:.4f}")
    check("(a) query<->CLIP cosine ROSE from ~0", cos_last > cos0 + 0.05, f"cos {cos0:.3f} -> {cos_last:.3f}")

    # ---- (c) teacher invisibility + inference without CLIP --------------
    pnames = [n for n, _ in model.named_parameters()]
    check("(c) teacher not in named_parameters",
          not any("_distill_teacher" in n or "teacher" in n for n in pnames))
    sd_keys = list(model.state_dict().keys())
    check("(c) no teacher weights in state_dict",
          not any("_distill_teacher" in k for k in sd_keys), f"{len(sd_keys)} keys")
    check("(c) learnable_query IS in state_dict (trained -> serialized)",
          "learnable_query" in sd_keys)
    with tempfile.TemporaryDirectory() as d:
        ...
        reloaded = VLMForCausalLM.from_pretrained(
            d, dtype=dtype, attn_implementation="sdpa"
        ).to(device)
        reloaded.eval()
        check("(c) reloaded model has NO teacher attached (inference path)",
              getattr(reloaded, "_distill_teacher", None) is None)
        check("(c) reloaded model rebuilt the queries + breen head",
        ...
        check("(c) forward works with no CLIP teacher (finite logits)",
    ...
    da = DataArguments(learnable_query_enabled=True, query_token="<query>",
                       query_token_index=-202, query_placement="after_image")
    turns = [{"from": "human", "value": "<image>\nWhat is this?"}]
    inject_query_placeholders(turns, n_images=1, data_args=da)
    check("(item7) after_image injects <query> right after <image>",
          "<image>\n<query>" in turns[0]["value"], turns[0]["value"])
    ...
    check("(item7) after_text appends <query> after the question",
