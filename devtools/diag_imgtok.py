...
def main():
    ...
    print(
        f"[diag] kind={kind} img_id={img_id} model_type={model.config.model_type if hasattr(model.config, 'model_type') else '?'}",
        flush=True,
    )
    ...
    if kind == "gemma":
        ...
        b = proc.apply_chat_template(
            msg, add_generation_prompt=True, tokenize=True, return_dict=True, return_tensors="pt"
        )
    ...
    vm = ids == img_id
    print(
        f"[diag] seq_len={len(ids)} n_image_tokens(vm.sum)={int(vm.sum())} "
        f"unique_ids_near_imgid={sorted(set(ids[max(0, int(vm.nonzero()[0, 0]) - 1) : int(vm.nonzero()[-1, 0]) + 2].tolist()))[:6] if vm.any() else 'NONE'}",
        flush=True,
    )
    keys = list(b.keys())
    ...
    if vm.any():
        ...
        print(
            f"[diag] L0 img-token norm={img_norm:.2f} txt-token norm={txt_norm:.2f} "
            f"img-token mean pairwise cos={mean_cos.item():.3f} (1.0=all identical)",
            flush=True,
        )
