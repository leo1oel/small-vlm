...
MODELS = [
    ...
    (
        ...
        None,
        "results_freeze_gemma.jsonl",
    ),
    (
        "SAIL · 7B",
        "native",
        "results_mat_sail.jsonl",
        "results_fusion_full_sail.jsonl",
        None,
        "results_freeze_sail.jsonl",
    ),
    (
        "LLaVA 1.5 · 7B",
        "encoder",
        "results_mat_llava.jsonl",
        "results_fusion_full_llava.jsonl",
        None,
        "results_freeze_llava.jsonl",
    ),
    (
        "LLaVA NeXT · 7B",
        "encoder",
    ...
    (
        "OneVision · 7B",
        "encoder",
        "results_mat_onevision.jsonl",
        "results_win_onevision.jsonl",
        None,
        None,
    ),
    (
...
def fig_c1():
    ...
    h = [
        plt.Line2D(
            ...
            mew=1.8,
            lw=1.5,
            label="native (no encoder)",
        ),
        plt.Line2D(
            [], [], color=C_ENC, marker="o", mfc="white", mew=1.8, lw=1.5, label="encoder-based"
        ),
    ]
        ...
        for j in range(N):
            ax.add_patch(
                Rectangle(
                    ...
                    x[j + 1] - x[j],
                    H,
                    color=plt.cm.Reds(0.08 + 0.86 * v[j]),
                    lw=0,
                )
            )
        if pb:
