...
def mat_summary(rows):
    ...
    return dict(
        ...
        u_last=ul,
        norm_img=ni,
        img_com_rel=com_rel(ui),
        txt_com_rel=com_rel(ut),
        img_early_share=img_early_share,
        txt_early_share=txt_early_share,
...
def main():
    print("=" * 110)
    print(
        f"{'model':16} {'fam':4} {'N':3} {'n/nc':9} {'intact':6} {'swap':6} "
    ...
    for name, fam, N, prebuf, ffile, mfile in ROSTER:
        ...
        if frows and any("cost" in r for r in frows):
            ...
            rec["func"] = {
                k: c[k]
                for k in ("R0", "intact", "swap", "com_rel", "retained", "rho", "n", "nc", "N")
            }
            fcom = f"{c['com_rel']:.3f}"
        ...
        master[name] = rec
        print(
            f"{name:16} {fam:4} {N:3} {ncs:9} {ist:6} {sw:6} {r0:6} {fcom:16} {icom:7} {ish:9} {rat:12}"
        )
    print("=" * 110)
    ...
    for jf in [
        "results_attn_llava_full.json",
        "results_attn_qwen_full.json",
        "results_attn_gemma.json",
        "results_sail_attn.json",
        "results_encoder_vlm_attn.json",
        "results_attn_mass.json",
    ]:
        p = ROOT / jf
        ...
        for k, v in d.items():
            if isinstance(v, dict) and "last2img" in v:
                img = v["last2img"]
                print(
                    f"  {k:34} n={v.get('n')} last->img mean={mean(img):.3f} "
                ...
                print(
                    f"  NEO-{k}(POPE) n={v.get('n')} last->vision mean={mean(l2v):.3f} "
