...
NATIVE = [("sail", "SAIL-7B"), ("gemma", "Gemma-4-12B"), ("mono", "Mono-InternVL-2B"),
          ("neo2b", "NEO1.0-2B"), ("neo9b", "NEO1.0-9B"),
          ("neo15_2b", "NEO1.5-2B"), ("neo15_9b", "NEO1.5-9B")]
# encoder-VLM = ships an external vision tower (SigLIP / InternViT / etc.)
ENCVLM = [("qwen", "Qwen2.5-VL-7B"), ("llava", "LLaVA-1.5-7B"), ("janus", "Janus-Pro-7B"),
          ("ivl8", "InternVL3.5-8B"), ("ivl30moe", "InternVL3.5-30B-A3B"),
...
def main():
    ...
    if cz:
        ...
        ax.text(0.015, max(cz) + 0.005, "real vision encoders (DINO / CLIP / SigLIP) — the ceiling",
                color="#1b7a3d", fontsize=9, style="italic", va="bottom")
    ax.axhline(0.001, color="#999", ls=":", lw=1)
    ax.text(0.99, 0.012, "chance (1/1000)", color="#999", fontsize=8, ha="right")

    for grp, models, cmap, mk in [("native", NATIVE, cm.Oranges, "o"), ("encvlm", ENCVLM, cm.Blues, "s")]:
        ...
        for j, (t, lb) in enumerate(present):
            ...
            ax.plot(x, y, "-" + mk, color=c, lw=2.2, ms=5, label=lb, zorder=3,
                    markeredgecolor="white", markeredgewidth=0.8)

    ax.set_xlim(0, 1); ax.set_ylim(0, 0.88)
    ...
    ax.set_title("Native VLMs grow a vision encoder INSIDE: image-token linear-probe climbs from\n"
                 "near-chance to the real-encoder ceiling.  Encoder-VLMs are flat-high (encoder fed in).",
                 fontsize=11.5)
    leg1 = ax.legend([h for h in ax.lines if h.get_label() in dict(NATIVE).values()],
                     [l for l in dict(NATIVE).values() if any(h.get_label() == l for h in ax.lines)],
                     title="native — built inside", loc="center right", frameon=False, fontsize=8.5,
                     title_fontsize=9)
    ax.add_artist(leg1)
    ax.legend([h for h in ax.lines if h.get_label() in dict(ENCVLM).values()],
              [l for l in dict(ENCVLM).values() if any(h.get_label() == l for h in ax.lines)],
              title="encoder-VLM — fed in", loc="lower right", frameon=False, fontsize=8.5, title_fontsize=9)
    ax.grid(alpha=0.22)
    fig.tight_layout()
