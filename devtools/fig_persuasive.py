...
FAM_LABEL = {
    "encoder": "encoder-based",
    "encoder-free": "encoder-free",
    "unified": "unified (no separate encoder)",
}
...
def fig_forest():
    rows = [
        (t, m) for t, m in D["models"].items() if t != "llava" and m.get("vmcbench")
    ]  # drop the explained outlier
    ...
    for i, (t, m) in enumerate(rows):
        ...
        ax.scatter(
            [v["q50"]],
            [i],
            s=130 if m["moe"] else 95,
            color=c,
            zorder=3,
            marker="D" if m["moe"] else "o",
            edgecolor="white",
            linewidth=1.3,
            label=lbl,
        )
    ...
    h += [
        Line2D(
            [], [], marker="D", color="#888", ls="", markeredgecolor="white", label="sparse MoE arm"
        )
    ]
    l += ["sparse MoE arm"]
    ...
    ax.text(
        0.012,
        -0.62,
        "LLaVA-1.5 (0.22) omitted — the one early case, a small-scale frozen-encoder "
        "recipe, not an architecture effect.",
        fontsize=7.6,
        color="#999",
        style="italic",
    )
...
def fig_invariance():
    ...
    axA.set_xticklabels(["VMCBench", "MMStar", "WorldBench"])
    axA.set_xlim(-0.3, 2.7)
    axA.set_ylim(0.2, 0.7)
    ...
    for i, dom in enumerate(doms):
        ...
        axB.bar(
            ...
            g[dom]["q50"],
            0.62,
            color=FAM_C["encoder-free"] if sig else "#d9d9d9",
            edgecolor="white",
            zorder=2,
        )
    ...
    axB.set_ylim(0, 0.7)
    axB.set_ylabel("relative fusion depth (sufmeanabl q50)")
...
def fig_triangulation():
    ...
    off = {
        ...
        "sail": (8, 2),
        "llava": (8, -2),
    }  # split the ivl8/gemma12 coincidence at (≈0.45, 0.42)
    for t, x, y, m in pts:
        c = FAM_C[m["family"]]
    ...
    ax.set_title(
        "Two orthogonal causal metrics agree\n"
...
def fig_build_before_fuse():
    ...
    for grp, ax, cmap in [("native", ax1, cm.Oranges), ("encoder", ax2, cm.Blues)]:
        ...
        for j, (pt, ft, label, _) in enumerate(ms):
            ...
            ax.plot(
                [fu], [-0.045], marker="^", ms=8, color=c, clip_on=False, zorder=5
            )  # fusion median tick
        ax.axvspan(
            0.35, 0.55, color="#555", alpha=0.08, zorder=0
        )  # mid-stack fusion band (where the ticks sit)
        ax.set_ylim(0, 0.80)
        ax.set_xlim(0, 1)
    ...
    ax1.set_title(
        "Vision is encoder-grade BEFORE mid-stack fusion reads it (▲ = each model's fusion depth)\n"
        "TOP: native models BUILD it inside (curves climb into the zone before ▲)  ·  "
        "BOTTOM: encoder-VLMs have it PRE-BUILT outside (flat-high from layer 0)",
        fontsize=10.5,
    )
    ax1.text(
        0.012,
        0.07,
        "native — built INSIDE (generative pre-training)",
        color="#a5531f",
        fontsize=9,
        fontweight="medium",
    )
    ax2.text(
        0.012,
        0.07,
        "encoder-VLM — built OUTSIDE (frozen vision encoder)",
        color="#2f4d7a",
        fontsize=9,
        fontweight="medium",
    )
