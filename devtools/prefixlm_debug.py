...
CKPT = (
    sys.argv[1]
    if len(sys.argv) > 1
    else ("/gscratch/scrubbed/leoym/small-vlm-outputs/sft-unified-bee-mix-prefixlm/checkpoint-3000")
)
...
def instrument(model):
    ...
    def logged(self, *a, **kw):
        ...
        if is_prefill and gm is not None and not state["done_probe"]:
            ...
            with torch.no_grad():
                ...
                for uc in (False, True):
                    o = self.model(
                        inputs_embeds=ie, attention_mask=gm, position_ids=pid, use_cache=uc
                    )
                    h = o[0] if isinstance(o, tuple) else o.last_hidden_state
                    lg = self.lm_head(h[0, -1]).float()
                    t = torch.topk(lg, 3)
                    print(
                        f"   [PROBE inner-model bidir mask use_cache={uc}] "
                        f"nan={bool(torch.isnan(lg).any())} top3_ids={t.indices.tolist()} "
                        f"vals={[round(x, 2) for x in t.values.tolist()]}",
                        flush=True,
                    )
        out = orig(self, *a, **kw)
        if is_prefill and hasattr(out, "logits") and out.logits is not None and state["calls"] < 4:
            ...
            t = torch.topk(lg, 5)
            print(
                f"[fwd #{state['calls']}] ie={tuple(ie.shape)} gen_mask={None if gm is None else tuple(gm.shape)} "
                f"top5_ids={t.indices.tolist()}",
                flush=True,
            )
        state["calls"] += 1
        return out
...
def main():
    ...
    a = generate_response(
        model, processor, query=query, images=image, temperature=0.0, max_new_tokens=32
    )
