...
for i, (lab, f, g) in enumerate(MODELS):
    ...
    if not q:
        print("skip (no quants):", lab); continue
    y = len(MODELS) - i
    ax.errorbar(q["q50"], y, xerr=[[q["q50"] - q["q25"]], [q["q75"] - q["q50"]]],
                fmt="o", ms=8, color=COL[g], ecolor=COL[g], elinewidth=1.5, capsize=3, zorder=5)
    yt.append(y); yl.append(lab)
    print("%-30s q50=%.2f [%.2f,%.2f]" % (lab, q["q50"], q["q25"], q["q75"]))
ax.set_yticks(yt); ax.set_yticklabels(yl, fontsize=9)
ax.set_xlim(0, 0.8); ax.set_ylim(0.3, len(MODELS) + 0.9)
...
fig.tight_layout()
for ext in ("png", "pdf"):
    fig.savefig(f"/mmfs1/gscratch/krishna/leoym/small-vlm/neo_report/fig_p1_generalization.{ext}", dpi=150)
