...
def draw_fusion_box(ax, q, y0=0.045, h=0.05):
    ...
    # box q25..q75
    ax.add_patch(Rectangle((q25, y0 - h), q75 - q25, 2 * h, facecolor=FUSE_C, alpha=0.30,
                           edgecolor=FUSE_C, lw=1.3, zorder=7))
    # median q50
...
def main():
    ...
    for k, (pt, ft, label, grp) in enumerate(MODELS):
        ...
        ax.text(0.02, ceil_hi + 0.006, "real-encoder ceiling", color=PROBE_C, fontsize=7.5,
                style="italic", va="bottom")
        ax.axhline(0.001, color="#aaa", ls=":", lw=0.9)
        pc = probe_curve(R, pt)
        if pc is not None:
            ax.plot(pc[0], pc[1], "-o", color=PROBE_C, lw=2.4, ms=5, zorder=5,
                    markeredgecolor="white", markeredgewidth=0.8, label="ImageNet linear-probe")
        q = fusion_quants(ft)
        if q is not None:
        ...
        ax.set_title(f"{label}\n{tag}", fontsize=10.5,
                     color="#b5530a" if grp == "native" else FUSE_C)

    for j in range(legend_idx, len(axes)):
        axes[j].axis("off")
    from matplotlib.lines import Line2D
    from matplotlib.patches import Patch
    axes[legend_idx].legend(handles=[
        Line2D([], [], color=PROBE_C, lw=2.4, marker="o", label="ImageNet linear-probe top-1\n(image rep → object identity)"),
        Patch(fc=PROBE_C, alpha=0.2, label="real vision-encoder ceiling\n(DINO / CLIP / SigLIP)"),
    ...
    ], loc="center", frameon=False, fontsize=10, title="Reading the figure", title_fontsize=12)
