...
def main():
    ...
    check("prior intact in normal build (CE well below uniform)",
          ce_norm < 0.5 * uniform, f"CE_norm={ce_norm:.2f} vs log|V|={uniform:.2f}")
    check("prior DESTROYED by random_init (CE near uniform)",
          ce_rand > 0.7 * uniform, f"CE_rand={ce_rand:.2f} vs log|V|={uniform:.2f}")
    check("random_init CE >> normal CE", ce_rand > ce_norm + 3.0,
          f"{ce_norm:.2f} -> {ce_rand:.2f}")
    ...
    check("embeddings re-initialized (large relative change)", rel > 0.5,
          f"rel L2 diff={rel:.3f}")
    ...
    check("connector params present + finite",
    ...
    check("image forward through connector works (finite feats)",
          torch.isfinite(feats[0]).all(), f"feat shape={tuple(feats[0].shape)}")
