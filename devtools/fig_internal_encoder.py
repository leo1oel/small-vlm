...
LABEL = {
    ...
    "llava": "LLaVA-1.5 (enc-VLM)",
    "qwen": "Qwen2.5-VL (enc-VLM)",
    "gemmarand": "Gemma-4 RANDOM-INIT",
    "llavarand": "LLaVA RANDOM-INIT",
}
...
def main():
    ...
    if has_in:
        ...
        ax2.set_xticks(x)
        ax2.set_xticklabels(
            [LABEL.get(t, t).split(" ")[0] for t in order], rotation=40, ha="right", fontsize=8
        )
        ax2.set_ylabel("ImageNet-1k linear-probe top-1")
        ax2.set_title(
            "Functional gold standard: native internal rep\nrises from pixel floor to ≥ encoder ceiling"
        )
