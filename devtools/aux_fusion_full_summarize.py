...
def summarize_arm(rows, N):
    ...
    marg = [
        max(retained[d - 1] - retained[d], 0.0) if d > 0 else max(R0 - retained[0], 0.0)
        for d in range(N)
    ]
    ...
    return dict(
        ...
        rho1=rho1,
        rho_floor=rho_floor,
        rho_min_layer=rho_min_layer,
        rho_consol=rho_consol,
    )
...
def main():
    ...
    print(
        f"N_layers={N}  base(n={B['n_all']}, causal={B['n_causal']})  "
    ...
    print(
        f"R0 signal  : base {B['R0']:+.3f}  aux {A['R0']:+.3f}  (intact-swap = usable image signal)"
    )
    ...
    print(
        f"functional CoM (layer)  : base {B['CoM']:.2f}  aux {A['CoM']:.2f}   (over marginal cost damage)"
    )
    print(
        f"single-layer nu net     : base {B['nu_net']:.4f}  aux {A['nu_net']:.4f}   (~0 = redundant, no single-layer bottleneck)"
    )
    print("\n== representational content-fusion rate rho ==")
    print(
        f"rho @ L1        : base {B['rho1']:.3f}  aux {A['rho1']:.3f}   (image content in text stream from layer 1)"
    )
    print(
        f"rho floor       : base {B['rho_floor']:.3f}@L{B['rho_min_layer']}  aux {A['rho_floor']:.3f}@L{A['rho_min_layer']}"
    )
    print(
        f"consolidation L : base {B['rho_consol']}  aux {A['rho_consol']}   (rho falls to peak-floor midpoint)"
    )

    print("\n== per-layer table ==")
    print(
        f"{'l':>3} | {'cost_b':>7} {'cnull_b':>7} {'ret_b':>6} | {'cost_a':>7} {'cnull_a':>7} {'ret_a':>6} | "
        f"{'nu_b':>7} {'nu_a':>7} | {'rho_b':>6} {'rho_a':>6}"
    )
    for l in range(N):
        print(
            f"{l + 1:>3} | {B['cost'][l]:7.3f} {B['cost_null'][l]:7.3f} {B['retained'][l]:+6.3f} | "
...
def make_figure(B, A, N, out):
    ...
    ax[0].plot(x, B["cost_null"], "--", color=cb, alpha=0.45, label="base null (swap img)")
    ax[0].plot(x, A["cost_null"], "--", color=ca, alpha=0.45, label="aux null (swap img)")
    for arm, c in ((B, cb), (A, ca)):
        if arm["L_f50"]:
            ax[0].axvline(arm["L_f50"], color=c, ls=":", alpha=0.6)
    ax[0].set_title("cost(d): cumulative decouplability\n(dotted = L_f50 half-signal depth)")
    ax[0].set_xlabel("blocked depth d  [0..d)")
    ax[0].set_ylabel("VMCBench acc")
    ...
    ax[1].axhline(B["R0"] / 2, color=cb, ls=":", alpha=0.4)
    ax[1].axhline(A["R0"] / 2, color=ca, ls=":", alpha=0.4)
    ax[1].set_title("retained(d) = real - null\n(usable image signal surviving the block)")
    ax[1].set_xlabel("blocked depth d  [0..d)")
    ax[1].set_ylabel("acc(real) - acc(swap)")
    ax[1].legend(fontsize=7)

    ax[2].plot(x, B["rho"], "-o", ms=3, color=cb, label="base")
    ax[2].plot(x, A["rho"], "-o", ms=3, color=ca, label="aux")
    ax[2].set_title("rho(l): representational fusion rate")
    ax[2].set_xlabel("layer l")
    ax[2].set_ylabel(r"$\|h(I)-h(I')\| / \|h(I)-h(\varnothing)\|$")
    ax[2].legend(fontsize=7)

    fig.suptitle(
        "Unified fusion metrics — bee-mix base vs aux (k=6, λ=0.5), VMCBench dev-1000", y=1.02
    )
    fig.tight_layout()
