...
def curve(rows, key):
    ...
    if R0 <= 0.02:
        return None
    ret = [
        acc(lambda r, d=d: r[key][d]) - acc(lambda r, d=d: r[key + "_null"][d]) for d in range(N)
    ]
    return dict(
        N=N,
        R0=R0,
        rn=[x / R0 for x in ret],
        nc=len(causal),
...
def lastrow_analysis():
    ...
    pairs = {
        "LLaVA-1.5-7B": ("results_fusion_full_llava.jsonl", "results_winlr_llava.jsonl"),
...
def main():
    res = {}
    print(
        f"{'model':14} {'fam':4} | suffix-rise: 20%   50%   80%  | prefix-fall: o80   c20  | window(50%rise->o80)"
    )
    print("-" * 105)
    for name, (fam, pfile, wfile) in MODELS.items():
        ...
        print(
            f"{name:14} {fam:4} |        {f(s20)}  {f(s50)}  {f(s80)} |        {f(po80)}  {f(pc20)} |  "
            f"{f((po80 - s50) if (po80 is not None and s50 is not None) else None)}"
            f"   (suf nc={suf['nc'] if suf else 0})"
        )
    (ROOT / "window_results.json").write_text(
        json.dumps({k: {kk: vv for kk, vv in v.items()} for k, v in res.items()}, indent=1)
    )
    ...
    for ax_i, (name, v) in enumerate(have):
        ...
        a.plot(
            ...
            color=c,
            lw=2,
            label="suffix: signal ALREADY fused\nby depth d (block [d..N))",
        )
        if "pre" in v:
            p = v["pre"]
    ...
    fig.suptitle(
        "Fusion WINDOW: solid rising = cumulative fused signal (onset); "
        "dashed falling = late-fusion capacity (completion)",
        fontsize=11,
    )
    fig.tight_layout()
