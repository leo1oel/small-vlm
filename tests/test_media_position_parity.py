...
def test_sandwich_does_not_duplicate_protected_audio():
    ...
    apply_image_position(convs, mode="sandwich", image_token=IMG, seed=0, protected_tokens=(AUD,))
    text = convs[0]["value"]
...
def test_collator_ok_when_sentinel_survives_truncation():
    ...
    collator = DataCollatorForSupervisedDataset(tokenizer=_Tok(8), media_token_ids=[IMG_ID, AUD_ID])
...
def test_collator_no_raise_when_within_limit():
    collator = DataCollatorForSupervisedDataset(tokenizer=_Tok(16), media_token_ids=[IMG_ID])
