...
from vlm.data.dataset import (
    _get_preprocess_tokenizer,  # pyright: ignore[reportPrivateUsage]
)
...
def test_cache_returns_same_object(tokenizer: Any) -> None:
    a = _get_preprocess_tokenizer(tokenizer, has_image=True)
    b = _get_preprocess_tokenizer(tokenizer, has_image=True)
    assert a is b


def test_image_variant_has_image_token(tokenizer: Any) -> None:
...
def test_original_tokenizer_not_mutated(tokenizer: Any) -> None:
    before = len(tokenizer)
    _get_preprocess_tokenizer(tokenizer, has_image=True)
    assert len(tokenizer) == before


def test_no_image_variant_is_distinct(tokenizer: Any) -> None:
    a = _get_preprocess_tokenizer(tokenizer, has_image=True)
